	...
	class ROM : UpdateHandler
	{
		...
		protected override void CircuitLogicUpdate()
		{
				...
				if (File.Exists(Directory.GetCurrentDirectory() + ROMFlash))
				{
					...
					if (data.Substring(0, 5) == "mmrom")
					{
						...
						if (data.Contains(";"))
						{
							...
							{
								format = 10;
								dpa = (2 ^ DataWidth).ToString().Length;
							}
							...
							if (continu)
							{
								if (readMode == "digit")
								{
									...
									int[] NewData = new int[data.Length / dpa];
									int j = 0;
									try
									{
								...
								else if(readMode == "lb")
								{
									...
									int[] NewData = new int[NewDataStr.Length];
									int j = 0;
									try 
									{ 
			...
			for(int i = 2; i < this.Inputs.Length; i++){
				Address = Address + (int)Math.Pow(2, i - 2) * Convert.ToInt32(this.Inputs[i].On);
			}
			int DataToOutput = 0;
